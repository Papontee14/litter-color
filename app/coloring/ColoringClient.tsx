...
export default function ColoringClient({ pictureId }: { pictureId: string }) {
  ...
  useEffect(() => {
    ...
    legacyImg.onload = () => {
      ...
      const oldSeg = findRegions({ width: SIZE, height: SIZE, data: virtual.data }, 180, original, legacyKey.split(':')[1]);
      const migrated = migrateFills(oldSeg, segmentation, oldFills);
      setWork({ fills: migrated.fills, history: [] });
      if (migrated.unmatched.length) setNotice('ภาพปรับปรุงใหม่ มีบางช่องที่ต้องเติมอีกครั้ง ผลงานเดิมยังเก็บไว้ในเครื่อง');
  ...
  function fill(region: number, chosen = color, erase = erasing) {
    if (!storageReady || !segmentation || region < 0 || region >= total) return; if ((erase && !work.fills[region]) || (!erase && work.fills[region] === chosen)) return;
